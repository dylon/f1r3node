...
/// SourceSpan-based version of FreeMap for use with rholang-rs parser types
///
...
impl<T: Clone> FreeMapSpan<T> {
    ...
    pub fn merge(&self, free_map: FreeMapSpan<T>) -> (FreeMapSpan<T>, Vec<(String, rholang_parser::SourceSpan)>) {
        let (acc_env, shadowed) = free_map.level_bindings.into_iter().fold(
            (self.level_bindings.clone(), Vec::new()),
            |(mut acc_env, mut shadowed), (name, free_context)| {
                acc_env.insert(
